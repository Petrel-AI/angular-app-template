...
describe('HasRoleDirective', () => {
  ...
  it('.isVisible should return true for Accountant directive when the Curent User is Accountant', () => {
    expect(target('accountant', spyOnCurrentUserServiceWithUserId(spyOn, 1, UserRole.Accountant)).isVisible).toBe(true);
  });

  it('.isVisible should return false for SystemAdministrator directive when the Curent User is a role below', () => {
    const auth = spyOnCurrentUserServiceWithUserId(spyOn, 1, UserRole.TaxPayer);
    expect(target('taxpayer', auth).isVisible).toBe(true);
    expect(target('accountant', auth).isVisible).toBe(false);
